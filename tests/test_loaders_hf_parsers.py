...
def test_parse_lcb_tests_preserves_testtype():
    row = {"tests": [{"input": "1", "output": "2", "testtype": "functional"}]}
    assert loaders._parse_lcb_tests(row) == [
        {"input": "1", "output": "2", "testtype": "functional"},
    ]


def test_parse_lcb_tests_type_alias():
    row = {"tests": [{"input": "in", "output": "out", "type": "stdin"}]}
    assert loaders._parse_lcb_tests(row) == [
        {"input": "in", "output": "out", "testtype": "stdin"},
    ]
