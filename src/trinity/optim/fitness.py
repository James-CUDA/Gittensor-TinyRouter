...
TRAINING-ONLY shaping mechanisms (see :class:`FitnessConfig`):
  ...
  * **hero_dense** — a HERO dense quality proxy (improvement #3, Stage A): each
    trajectory's self-consistency (fraction of its answer-bearing turns agreeing
    with the committed answer) is min-max normalized WITHIN the correct and
    incorrect buckets and added as a small in-bucket bonus, so the optimizer gets
    gradient among equally-correct (or equally-wrong) candidates while correctness
...
def hero_quality(traj) -> float:
    ...
    Of the trajectory's *answer-bearing* turns (``reward.has_answer``), the fraction
    whose answer agrees with the committed final answer (``reward.committed_answer``).
    A trajectory whose turns all reach the same answer is high quality (``1.0``); one
    where the turns disagree is lower; a trajectory that never produced a parseable
    answer is ``0.0``. Pure and torch-free.

    This is the dense quality signal improvement #3 (Stage A) calls for. It shapes
    only TRAINING fitness (via :func:`hero_bucket_bonus`); the eval scorer never
    sees it.
    """
    ...
    total = 0
    agree = 0
    for tr in getattr(traj, "turns", None) or []:
        txt = getattr(tr, "processed_output", "") or ""
        if not _reward.has_answer(benchmark, txt):
            continue
        total += 1
        if _answers_agree(benchmark, txt, committed):
            agree += 1
    if total == 0:
        return 0.0
    return agree / total
